    ...
    private string GetModifierText()
    {
        ...
        foreach(StatModifier modifier in _character._statsModifiers)
        {
            ...
            else
            {
                ...
                {
                    message += $"<color=#F3BEBE>{modifier._stat} : -{modifier._stage * 10}% </color>" + Environment.NewLine;
                }
                else
                {
                    message += $"<color=#0000FF>{modifier._stat} : +{modifier._stage * 10}%</color>" + Environment.NewLine;
                }
            }
        }
        return message;
